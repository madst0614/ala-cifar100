"""Exp3: Fine-grained exploration around exp_b's sqrt scaling.

exp2 showed exp_b (σ(z/sqrt(num_classes))) was the only variant that beat
baseline CE (59.25% vs 58.36%), but policy entropy collapsed to 0 and Phi
barely moved.  This experiment fixes sqrt scaling and searches for conditions
where Phi moves meaningfully while maintaining/improving accuracy.

Key addition: CE blend — loss = ce_blend*CE + (1-ce_blend)*adaptive_loss.
This keeps CE gradients alive for stable model training while letting the
adaptive component provide Phi-driven signal.

Warmup checkpoint reused from results/exp2_warmup_checkpoint.pt (no warmup).
"""
...
CONFIGS = OrderedDict({
    "exp3_a": {
        "description": "sqrt + clamp[-0.3,0.3] + delta=0.05",
        "use_ala": True,
        ...
        "selective_k": None,
        "reward_type": "sign",
        "ce_blend": 0.0,  # adaptive loss only
    },
    "exp3_b": {
        "description": "sqrt + clamp[-0.1,0.1] + delta=0.05",
        "use_ala": True,
        "sigma_scale": "sqrt_num_classes",
        "clamp_range": (-0.1, 0.1),
        "delta_scale": 0.05,
    ...
    "exp3_c": {
        "description": "sqrt + CE blend 0.5 + clamp[-1,1] + delta=0.01",
        "use_ala": True,
        "sigma_scale": "sqrt_num_classes",
        "clamp_range": (-1.0, 1.0),
        "delta_scale": 0.01,
        "selective_k": None,
        "reward_type": "sign",
        "ce_blend": 0.5,  # loss = 0.5*CE + 0.5*adaptive
    },
    "exp3_d": {
        "description": "sqrt + CE blend 0.5 + clamp[-0.3,0.3] + delta=0.05",
        "use_ala": True,
        "sigma_scale": "sqrt_num_classes",
        "clamp_range": (-0.3, 0.3),
        "delta_scale": 0.05,
...
# ---------------------------------------------------------------------------
# Phase 1: Single experiment runner (with CE blend support)
# ---------------------------------------------------------------------------
...
def run_experiment(
    ...
    phi_snapshot_epochs = {100, 150, 200}
    ...
    while global_step < total_steps:
        ...
        for j in range(1, K + 1):
            ...
            try:
                ...
                # Early stopping: if val_acc drops below warmup level by 15%+
                ...
                if current_epoch in phi_snapshot_epochs:
                    torch.save(
                        ...
                        f"results/exp3_{name}_phi_{current_epoch}.pt",
                    )
            ...
            # SGD step with CE blend
            model.train()
            optimizer.zero_grad()
            logits = model(inputs)

            if ce_blend > 0.0:
                loss_ce = ce_loss_fn(logits, targets_batch)
                loss_ada = adaptive_loss(logits, targets_batch)
                loss = ce_blend * loss_ce + (1.0 - ce_blend) * loss_ada
            else:
...
def save_individual_log(name, result):
    path = f"results/exp3_{name}_log.txt"
...
def generate_summary(results):
    ...
    for name, result in results.items():
        if not result["epoch_logs"] or "FAIL" in result["status"] or "EARLY" in result["status"]:
            status_str = result["status"]
            lines.append(f"{name:<17} | {status_str:>14} | {'-':>13} | "
                         f"{'-':>13} | {'-':>18} | {'-':>10}")
            continue
    ...
    text = "\n".join(lines)
    print("\n" + text)
    with open("results/exp3_summary.txt", "w") as f:
        f.write(text + "\n")
    print("\nSummary saved to results/exp3_summary.txt")
...
def generate_plots(results):
    ...
    plt.tight_layout()
    plt.savefig("results/exp3_comparison.png", dpi=150)
    plt.close()
    print("Comparison plot saved to results/exp3_comparison.png")
...
def main():
    ...
    # Warmup checkpoint must exist (from exp2)
    ckpt_path = "results/exp2_warmup_checkpoint.pt"
    assert os.path.exists(ckpt_path), (
        "Run exp2.py first to generate warmup checkpoint at " + ckpt_path
    )
    print(f"Using warmup checkpoint: {ckpt_path}")
